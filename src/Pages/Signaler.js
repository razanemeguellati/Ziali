...
const Signaler = () => {
  ...
  const handleSubmit = (event) => {
    ...
    const probleme = {
      nature,
      wilaya: Wilayas.find((JSONwilaya) => JSONwilaya.name === wilaya).id,
      commune: Communes.find((JSONcommune) => JSONcommune.name === commune)
        .post_code,
      description,
      owner,
  ...
  const getLocationJs = () => {
    navigator.geolocation.getCurrentPosition((position) => {
      ...
      setCurrLocationJs({ latitude: latitude, longitude: longitude });
    });
  };
  ...
  function handleCurrPositionJs() {
    ...
    const longitude = currLocationJs.longitude;
    setCurrLocation({ latitude: latitude, longitude: longitude });
    alert(" Votre position a été enregistrer ");
  }

  useEffect(() => {
    setCurrLocation(currLocationJs);
  }, [currLocationJs]);
  useEffect(() => {
  ...
  const UpdateMap = () => {
    ...
    useEffect(() => {
      if (currLocation.latitude === null || currLocation.longitude === null)
        return;
      console.log("useeffrect", currLocation);
      map.setView([currLocation.latitude, currLocation.longitude]);
    }, [currLocation, map]);
    useMapEvents({
      click: (e) => {
        ...
        setMapCurrLocation({ latitude: latitude, longitude: longitude });
      },
    });
  };
  ...
  return (
    <div className=" mt-6 lg:mt-0 flex flex-col justify-center items-center space-y-2 ">
      <h1 className=" text-2xl font-bold ">
        {" "}
        Signaler un <b className="text-blue">problème</b>{" "}
      </h1>
      <p className="text-[0.75rem] text-black opacity-40">
        {" "}
        Veuillez remplir ces champs et valider le signalement{" "}
      </p>

      <form
        onSubmit={handleSubmit}
        className="w-full flex flex-col justify-center items-start space-y-3 pt-5 pb-10 lg:ml-14"
      >
        <div
          id="nature-div"
          className="flex flex-row w-full pl-10 space-x-4 justify-left "
        >
          ...
          <div className="flex flex-col lg:flex-row lg:justify-around w-[60%] ">
            <div className="flex flex-row items-center justify-left space-x-2">
              ...
              <label
                htmlFor="type1"
                className="text-blue text-[0.8rem] lg:text-[1rem]"
              >
                {" "}
                Fuite d'eau{" "}
              </label>
            ...
            <div className="flex flex-row items-center justify-left space-x-2">
              ...
              <label
                htmlFor="type2"
                className="text-red text-[0.8rem] lg:text-[1rem]"
              >
                {" "}
                Incendie{" "}
              </label>
            ...
            <div className="flex flex-row items-center justify-left space-x-2">
              <input
                ...
                type="radio"
                name="nature"
                value="Autre problème"
                id="type3"
                onChange={(e) => setNature(e.target.value)}
              />
              <label
                htmlFor="type3"
                className="text-green text-[0.8rem] lg:text-[1rem]"
              >
                {" "}
                Autre problème{" "}
              </label>
        ...
        <div className="flex flex-col lg:flex-row gap-3 w-[50%] ">
          <div
            id="wilaya-div"
            className="flex flex-row w-full  px-10 space-x-8 justify-between items-center lg:justify-start"
          >
            <label htmlFor="type1" className=" text-[0.85rem] lg:text-[1rem]">
              {" "}
              Wilaya: {wilaya}{" "}
            </label>
            ...
            <input
              ...
              name="wilaya"
              placeholder="Wilaya"
              className="md:w-[140px] lg:w-[200px] w-40 h-8 p-2 border-2 border-green rounded-full text-xs"
              value={wilaya}
            ...
            <datalist id="wilayas">
              {Wilayas.map((w) => {
                return <option value={w.name} className="w-full" />;
              })}
            </datalist>
          </div>
        ...
        <div className="flex flex-col lg:flex-row items-center lg:items-start justify-center lg:justify-start pt-5 lg:gap-20 w-full">
          <div
            id="Map-div"
            className="lg:order-2 flex flex-col  px-3 justify-center items-center lg:w-[55%]"
          >
          ...
          <div className=" flex flex-col items-center lg:space-y-3 space-y-5  ">
            ...
            <div
              id="Photos-div"
              className="w-full flex flex-col px-7 justify-center items-left space-y-1"
            >
            ...
            <div
              ...
              className="w-full flex flex-col px-7 justify-center items-left space-y-1"
            >
              <label htmlFor="commentaire" className="px-2">
                {" "}
              ...
              <textarea
                ...
                className="border-2 border-green rounded-2xl p-4 lg:p-5 lg:w-full lg:h-40"
              ></textarea>
            </div>
            ...
            <div className="flex flex-col space-y-4 lg:space-x-5 lg:flex-row w-full items-center justify-center ">
              ...
              <label
                for="submit"
                className="submit-button cards py-2 px-14 w-fit flex justify-center items-center gap-2 cursor-pointer hover:scale-110 bg-green rounded-full text-black font-semibold shadow-xl shadow-lightgris"
              >
                <span>Valider</span>
              </label>

              <button className="cards py-2 px-12 w-fit flex justify-center items-center gap-2 cursor-pointer hover:scale-110 bg-red rounded-full text-black font-semibold shadow-xl shadow-lightgris">
                {" "}
                Annuler{" "}
              </button>
            </div>
